package shame.nazuna.client.modules.impl.render;
   ...
   extends Module
 {
   ...
   private final Identifier bubbleTexture = Identifier.method_60655("astra", "textures/hitbubble/bubble.png");
   ...
   public void onAttack(EventAttackEntity event) {
     ...
     if (event == null || event.getTarget() == null)
       return;  Entity Entity = event.getTarget(); if (Entity instanceof LivingEntity) { living = (LivingEntity)Entity; } else { return; }
      if (event.getPlayer() == null)
       return; 
     Vec3d sideDir = getHitSideDirection(living, event.getPlayer().method_19538());
   ...
   private void renderSingleBubble(MatrixStack stack, Vec3d cameraPos, HitBubble bubble, long now) {
     ...
     Vec3d rel = bubble.pos().method_1020(cameraPos);
     ...
     stack.method_22907(RotationAxis.field_40716.rotationDegrees(bubble.sideYaw()));
     stack.method_22907(RotationAxis.field_40714.rotationDegrees(-210.0F));
     stack.method_22907(RotationAxis.field_40718.rotationDegrees(rotation));
   ...
   private Vec3d getHitSideDirection(LivingEntity target, Vec3d attackerPos) {
     Vec3d dir = attackerPos.method_1020(target.method_19538());
     dir = new Vec3d(dir.field_1352, 0.0D, dir.field_1350);
     if (dir.method_1027() < 1.0E-4D) {
       Vec3d fallback = target.method_5720();
       dir = new Vec3d(fallback.field_1352, 0.0D, fallback.field_1350);
     } 
     if (dir.method_1027() < 1.0E-4D) dir = new Vec3d(0.0D, 0.0D, 1.0D); 
     return dir.method_1029();
   }
   
   private Vec3d getHitPosition(LivingEntity target, Vec3d sideDir) {
     Vec3d head = new Vec3d(target.method_23317(), target.method_23318() + target.method_17682() + 0.18D, target.method_23321());
     return head.method_1019(sideDir.method_1021(0.1D));
   }
   private static final class HitBubble extends Record { private final Vec3d pos; private final long spawnTime; private final float spinSeed; private final float sideYaw;
     private HitBubble(Vec3d pos, long spawnTime, float spinSeed, float sideYaw) { this.pos = pos; this.spawnTime = spawnTime; this.spinSeed = spinSeed; this.sideYaw = sideYaw; } public final String toString() { // Byte code:
       //   0: aload_0
       ...
       //   0	7	0	this	Lshame/astra/client/modules/impl/render/HitBubbles$HitBubble; } public Vec3d pos() { return this.pos; } public final int hashCode() { // Byte code:
       //   0: aload_0
