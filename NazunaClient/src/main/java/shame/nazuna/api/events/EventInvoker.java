...
public class EventInvoker {
    ...
    private static final boolean PERF_DEBUG = Boolean.parseBoolean(System.getProperty("nazuna.perf.debug", "false"));
    private static final long SLOW_HANDLER_NANOS = Long.getLong("nazuna.perf.handlerMs", 8L).longValue() * 1000000L;
    private static final long SLOW_EVENT_NANOS = Long.getLong("nazuna.perf.eventMs", 18L).longValue() * 1000000L;
    private static final long WARN_COOLDOWN_NANOS = Long.getLong("nazuna.perf.cooldownMs", 1000L).longValue() * 1000000L;
            ...
            for (Method method : listener.getClass().getDeclaredMethods()) {
                if (method.isAnnotationPresent(EventLink.class)) {
                    ...
                    if (parameters.length == 1 && Event.class.isAssignableFrom(parameters[0])) {
                        ...
                        method.setAccessible(true);
                        rebuilt.computeIfAbsent(eventClass, k -> new ArrayList<>())
    ...
    private static void logSlowHandler(Event event, Invocation invocation, long elapsedNanos) {
        ...
        System.out.println(String.format(Locale.ROOT, "[PerfDebug] Slow handler: %s -> %s#%s took %.2f ms",
            eventName, listenerName, methodName, elapsedNanos / 1000000.0D));
    }
    ...
    private static void logSlowEvent(Event event, long elapsedNanos, int invocationCount) {
        ...
        System.out.println(String.format(Locale.ROOT, "[PerfDebug] Slow event: %s took %.2f ms for %d handlers",
            eventName, elapsedNanos / 1000000.0D, invocationCount));
    }
    ...
    private static final class Invocation {
        ...
        public Invocation(Object listener, Method method, int priority) {
